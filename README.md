...
## Filmbewertungen absenken (einmalige Korrektur)

Eine Korrektur an einem einzigen Maßstab: Waren die **Filme** über die
Jahre zu mild bewertet, zieht diese Aktion bei **jedem bewerteten Film**
**0,33** von **jedem der sieben Kriterien** ab. Sie steht im Daten-Panel
(Zahnrad im Kopfbereich) unter **„Filmbewertungen"** —
`FILM_SENKUNG_PRO_KRITERIUM` in `src/App.jsx`.

**Warum das die Endnote um 0,25 senkt.** Die Kriteriengewichte ergeben
in Summe 1 (siehe *Berechnung*). Fällt jedes Kriterium um 0,33, fällt
die Kriterien-Note um ebenfalls 0,33 — und weil sie mit 75 % in die
Endnote eingeht:

```
0,75 × 0,33 = 0,2475  →  die 0,25, mit der die Endnote dasteht
```

**Was unberührt bleibt.** Alles außer den Filmkriterien:
...
| Vorgemerkte Filme | haben keine Bewertung und stehen deshalb in keiner der Listen |

**Hier wird gekappt — anders als beim Verrechnen.** Ein Kriterium darf
nicht unter 0 fallen. Ein Film, dessen schwächstes Kriterium darunter
käme, wird deswegen **nicht übersprungen**; sein Kriterium landet auf
0, und seine Endnote fällt entsprechend **weniger weit als 0,25**. Genau
diese Filme nennt die Vorschau **namentlich**, mit der Endnote davor und
danach, dem tatsächlichen Abzug und den gekappten Kriterien — damit
klar ist, wo die Verschiebung nicht voll durchschlägt.

**Erst Vorschau, dann schreiben.** Der erste Klick rechnet nur und fasst
nichts an (`filmSenkungsPlan`). Er zeigt:

* wie viele Filme betroffen sind,
* bei wie vielen mindestens ein Kriterium unter 0 fiele,
* und welche das sind.

Geschrieben wird erst nach einem zweiten, ausdrücklichen Klick
(„N Filme absenken"). Gespeichert wird Film für Film wie eine normale
Bewertungsänderung (`PUT /api/items?id=…`), nacheinander; `elo`, `duels`
und `siege` gehen unverändert aus dem Eintrag mit. Scheitert einer,
laufen die übrigen trotzdem durch.

**Gerechnet wird auf dem Hundertstel**, nicht auf der 0,1-Stufe der
Schieber (`aufSammelSchritt`, dieselbe Stufe wie bei der
Sammelfunktion): 0,33 liegt nicht auf der Schieberstufe, und ein Runden
darauf würde die zugesagten 0,25 verfehlen.

**Der Rundungsrest wird genannt, nicht verschwiegen.** Ein Kriterium
wandert immer um genau 0,33 (oder auf 0). Die Endnoten-Formel rundet
auf ihrem Weg aber mehrfach auf das Hundertstel — Kriterien-Note,
Klammerteil, Endnote mit Zuschlag. In der Anzeige steht deshalb meist
−0,25, gelegentlich −0,24 oder −0,26; weiter als **0,02**
(`FILM_SENKUNG_MAX_REST`) wandert es nicht. Geprüft wird das in
`test/film-senkung.test.mjs` — an einem vollständigen Datensatz mit
erspielten Elo-Zahlen und zusätzlich an über hunderttausend gestreuten
Fällen.

**Als einmalige Korrektur gedacht.** Der Knopf bleibt danach stehen und
lässt sich erneut drücken — ein zweiter Durchlauf senkt ein zweites Mal.
Der Kasten sagt das dazu. Rückgängig machen lässt sich die Aktion nicht
automatisch; von Hand zurückstellen geht jederzeit.
