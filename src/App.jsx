...
/* ----------------------------------------------------------------
   Einmalige Absenkung aller Filmbewertungen

   Eine Korrektur an einem einzigen Massstab: Filme waren ueber die
   Jahre zu mild bewertet. Deshalb geht auf JEDES der sieben
   Filmkriterien jedes bewerteten Films derselbe Abzug von
   FILM_SENKUNG_PRO_KRITERIUM.

   Warum das die Endnote um genau ein Viertel senkt:

     Endnote = 0,75 x Kriterien-Note + 0,25 x Bauchgefuehl + Zuschlag

   Die Kriteriengewichte ergeben in Summe 1 (siehe AV_CRITERIA).
   Faellt jedes Kriterium um 0,33, faellt die Kriterien-Note um
   ebenfalls 0,33 — und die Endnote um 0,75 x 0,33 = 0,2475, also um
   die 0,25, mit der sie dasteht. Uebrig bleibt allein der
   Rundungsrest: Die Endnoten-Formel rundet auf dem Weg mehrfach auf
   das Hundertstel (Kriterien-Note, Klammerteil, Endnote mit
   Zuschlag), und jede dieser Rundungen kann einen halben Cent
   verschieben. In der Anzeige steht deshalb meist −0,25, gelegentlich
   −0,24 oder −0,26; weiter als FILM_SENKUNG_MAX_REST wandert es
   nicht.

   Was die Aktion NICHT anfasst:

     - jede andere Kategorie. Sie gilt ausschliesslich fuer Filme.
     - das Bauchgefuehl. Es bleibt Ziffer fuer Ziffer stehen.
     - Elo, Duellzahl, Siege und damit den Duell-Zuschlag. Anders als
       die Sammelfunktion darueber setzt sie die Elo NICHT zurueck —
       der Zuschlag eines Films steht danach unveraendert neben der
       neuen Kriterien-Note.
     - Staffeln. Filme haben keine (siehe SEASON_CATEGORIES).

   Gekappt wird, anders als beim Verrechnen: Ein Kriterium darf nicht
   unter 0 fallen, und ein Film, dessen schwaechstes Kriterium
   darunter kaeme, wird deswegen nicht uebersprungen — er faellt nur
   weniger weit. Bei ihm ist die Senkung der Endnote also geringer
   als 0,25. Diese Faelle stehen in der Vorschau namentlich, damit
   klar ist, wo die Verschiebung nicht voll durchschlaegt.

   Gerechnet wird auf dem Hundertstel, wie bei der Sammelfunktion
   (siehe aufSammelSchritt): 0,33 liegt nicht auf der 0,1-Stufe der
   Schieber, und ein Runden darauf wuerde die zugesagten 0,25
   verfehlen.

   Geschrieben wird erst nach ausdruecklicher Bestaetigung: Die
   Vorschau (filmSenkungsPlan) rechnet nur, sie fasst keinen Eintrag
   an. Und sie ist als einmalige Korrektur gedacht — ein zweiter
   Durchlauf senkt ein zweites Mal.
   ---------------------------------------------------------------- */

/* Die Kategorie, um die es geht — und nur um sie. */
const FILM_SENKUNG_KATEGORIE = "movie";

/* Der Abzug je Kriterium. */
const FILM_SENKUNG_PRO_KRITERIUM = 0.33;

/* Woraus sich die Senkung der Endnote ergibt: der Abzug mal dem
   Anteil, mit dem die Kriterien in die Endnote eingehen. Die Zahl
   steht nirgends fest, sie wird gerechnet — laeuft der Anteil je
   auseinander, wandert die Anzeige mit. */
const FILM_SENKUNG_ENDNOTE = FILM_SENKUNG_PRO_KRITERIUM * VERRECHNEN_ANTEIL_KRITERIEN;

/* Wie weit die Senkung einer Endnote hoechstens von diesem Wert
   abweichen darf. Anders als bei der Sammelfunktion ist das keine
   Bedingung, an der ein Eintrag scheitern koennte — es ist die
   Zusage, gegen die die Tests den ganzen Datensatz pruefen. Sie
   stammt allein aus den Rundungen der Endnoten-Formel; ein Kriterium
   wandert nie um etwas anderes als die 0,33. */
const FILM_SENKUNG_MAX_REST = 0.02;

/**
 * Ein Film, durchgerechnet.
 *
 * Aufbau wie sammelVerrechnung: zurueck kommt immer ein Objekt,
 * `moeglich` sagt, ob der Film abgesenkt wird, und `grund` warum
 * nicht. Angefasst wird hier nichts — `entwurf` ist ein Vorschlag,
 * mehr nicht.
 *
 * `gekappt` sagt, ob mindestens ein Kriterium an der 0 angestossen
 * ist; `gekappteKriterien` nennt sie beim Namen. Bei diesen Filmen
 * ist `abweichung` kleiner als die zugesagten 0,25.
 */
function filmSenkung(entry) {
  const category = FILM_SENKUNG_KATEGORIE;
  const kriterien = criteriaFor(category);
  const noteVorher = entryScore(entry, category);

  const grundlage = {
    id: entry.id, category, titel: entry.title || "", noteVorher,
    proKriterium: FILM_SENKUNG_PRO_KRITERIUM,
  };
  const offen = (grund) => ({
    ...grundlage, moeglich: false, grund, gekappt: false, gekappteKriterien: [],
    noteNachher: null, abweichung: null, entwurf: null,
  });

  if (entry.watchlist === true) return offen("Vorgemerkt — es gibt noch keine Bewertung.");
  /* Filme haben keine Staffeln (siehe SEASON_CATEGORIES). Stuenden bei
     einem doch welche, wuerde dieser Weg sie uebergehen — dann lieber
     stehen lassen und benennen. */
  if (hasSeasons(entry)) return offen("Der Eintrag hat Staffeln.");
  if (typeof noteVorher !== "number") return offen("Der Eintrag hat keine Endnote.");

  const werte = kriterien.map((c) => (entry.values ? entry.values[c.key] : undefined));
  if (werte.some((w) => typeof w !== "number")) return offen("Dafür fehlen bewertete Kriterien.");

  /* Nur die Kriterien dieser Kategorie werden angefasst; was sonst in
     `values` steht, bleibt unberuehrt stehen. Das Bauchgefuehl
     ebenso. */
  const neueWerte = { ...(entry.values || {}) };
  const gekappteKriterien = [];
  for (const c of kriterien) {
    const roh = neueWerte[c.key] - FILM_SENKUNG_PRO_KRITERIUM;
    if (roh < BEWERTUNG_MIN - VERRECHNEN_TOLERANZ) gekappteKriterien.push(c.label);
    neueWerte[c.key] = aufSammelSchritt(Math.max(BEWERTUNG_MIN, roh));
  }

  const entwurf = { values: neueWerte, personal: entry.personal, seasons: [] };

  /* Was danach wirklich dastuende — ueber dieselbe Rechnung wie
     ueberall sonst. Die Elo bleibt, wo sie ist: der Duell-Zuschlag
     geht unveraendert mit in die neue Endnote. */
  const danach = { ...entry, ...entwurf };
  const noteNachher = entryScore(danach, category);
  const abweichung = Math.round((noteNachher - noteVorher) * 100) / 100;

  return {
    ...grundlage, moeglich: true, grund: "",
    gekappt: gekappteKriterien.length > 0, gekappteKriterien,
    noteNachher, abweichung, entwurf,
  };
}

/**
 * Die Vorschau ueber alle Filme. Rechnet nur.
 *
 * `items` ist der Bestand nach Kategorien, so wie ihn die App haelt.
 * Angesehen wird allein die Film-Liste. Zurueck kommen drei Listen in
 * der Reihenfolge des Bestands:
 *
 *   voll          — die Senkung schlaegt voll durch (−0,25).
 *   gekappt       — mindestens ein Kriterium stiesse an die 0; die
 *                   Endnote faellt weniger weit. Mit Titel, damit es
 *                   sich namentlich auflisten laesst.
 *   uebersprungen — bleibt unveraendert, mit Begruendung.
 *
 * Vorgemerkte Filme stehen in keiner der drei Listen: Sie haben keine
 * Bewertung, an der sich etwas senken liesse.
 */
function filmSenkungsPlan(items) {
  const voll = [];
  const gekappt = [];
  const uebersprungen = [];
  for (const entry of (items && items[FILM_SENKUNG_KATEGORIE]) || []) {
    if (entry && entry.watchlist === true) continue;
    const vorgang = filmSenkung(entry);
    if (!vorgang.moeglich) uebersprungen.push(vorgang);
    else if (vorgang.gekappt) gekappt.push(vorgang);
    else voll.push(vorgang);
  }
  return {
    voll, gekappt, uebersprungen,
    aenderbar: voll.concat(gekappt),
    betroffen: voll.length + gekappt.length,
  };
}

/**
 * Die Anfrage, mit der ein einzelner Film gespeichert wird: eine ganz
 * normale Bewertungsaenderung. `elo`, `duels` und `siege` gehen
 * unveraendert aus dem Eintrag mit — die Duell-Wertung bleibt damit
 * genau so stehen, wie sie war.
 */
function filmSenkungsAnfrage(entry, vorgang) {
  return {
    ...entry,
    category: vorgang.category,
    values: vorgang.entwurf.values,
...
/* Die Zeile ueber der Vorschau — wie viele Filme betroffen sind und
   bei wie vielen gekappt wird. */
function filmSenkungsVorschauText(plan) {
  if (!plan || !plan.betroffen) return "Es gibt keinen bewerteten Film, der sich senken ließe.";
  const filme = plan.betroffen === 1 ? "1 Film" : plan.betroffen + " Filme";
  if (!plan.gekappt.length) {
    return filme + " betroffen, bei keinem fällt ein Kriterium unter " +
      stufenText(BEWERTUNG_MIN) + ".";
  }
  const bei =
    plan.gekappt.length === 1
      ? "bei 1 Film würde mindestens ein Kriterium unter " + stufenText(BEWERTUNG_MIN) + " fallen"
      : "bei " + plan.gekappt.length + " Filmen würde mindestens ein Kriterium unter " +
        stufenText(BEWERTUNG_MIN) + " fallen";
  return filme + " betroffen, " + bei + " — dort wird gekappt:";
}
...
/**
 * Der Kasten im Daten-Panel: „Alle Filmbewertungen absenken".
 *
 * Aufbau wie SammelVerrechnen, und aus demselben Grund: Ohne `plan`
 * steht hier nur der Knopf, der die Vorschau rechnet — geschrieben
 * wird dabei nichts. Mit `plan` steht da, wie viele Filme betroffen
 * sind, bei welchen gekappt wird und wie weit ihre Endnote wirklich
 * faellt; erst darunter der Knopf, der es tut.
 */
function FilmSenkung({ plan, busy, ergebnis, onVorschau, onSenken, onAbbrechen }) {
  const hinweis = { fontSize: 11, color: "#77746c", marginTop: 8, lineHeight: 1.5 };
  const knopf = {
    width: "100%", padding: "12px", borderRadius: 8, fontSize: 14,
    background: "transparent", color: "var(--accent, #C9A227)",
  ...
  if (!plan) {
    return (
      <div>
        <button type="button" onClick={onVorschau} disabled={busy} style={knopf}>
          Alle Filmbewertungen absenken
        </button>
        <div style={hinweis}>
          Zieht bei jedem bewerteten Film {notenText(FILM_SENKUNG_PRO_KRITERIUM)} von jedem
          der Kriterien ab; die Endnote fällt dadurch um {notenText(FILM_SENKUNG_ENDNOTE)}.
          Bauchgefühl, Duell-Zuschlag und alle anderen Kategorien bleiben unberührt. Kein
          Kriterium fällt unter {stufenText(BEWERTUNG_MIN)} — wo gekappt wird, fällt die
          Endnote weniger weit; diese Filme nennt die Vorschau beim Namen. Zuerst kommt
          eine Vorschau — geschrieben wird erst nach deiner Bestätigung.
        </div>
        <div style={{ ...hinweis, marginTop: 6 }}>
          Als einmalige Korrektur gedacht: Ein zweiter Durchlauf senkt ein zweites Mal.
        </div>
        {ergebnis && (
          <div style={{ ...hinweis, color: "#9A968C", marginTop: 10 }}>{ergebnis}</div>
        )}
  ...
  return (
    <div>
      <div style={{ fontSize: 13, color: "#EDEAE3", lineHeight: 1.5 }}>
        {filmSenkungsVorschauText(plan)}
      </div>

      {plan.gekappt.length > 0 && (
        <ul style={{ margin: "10px 0 0", padding: 0, listStyle: "none" }}>
          {plan.gekappt.map((v) => (
            <li key={v.id} style={{ fontSize: 12, color: "#9A968C", lineHeight: 1.5, marginBottom: 6 }}>
              <span style={{ color: "#EDEAE3" }}>{v.titel}</span>
              <span style={{ color: "#55524c" }}>
                {" · "}{notenText(v.noteVorher)} → {notenText(v.noteNachher)}
              </span>
              <div style={{ fontSize: 11.5, color: "#77746c" }}>
                {betragText(v.abweichung)} statt {betragText(-FILM_SENKUNG_ENDNOTE)} — gekappt
                bei: {v.gekappteKriterien.join(", ")}
              </div>
            </li>
          ))}
        </ul>
      ...
      {plan.uebersprungen.length > 0 && (
        <ul style={{ margin: "10px 0 0", padding: 0, listStyle: "none" }}>
          {plan.uebersprungen.map((v) => (
            <li key={v.id} style={{ fontSize: 12, color: "#9A968C", lineHeight: 1.5, marginBottom: 6 }}>
              <span style={{ color: "#EDEAE3" }}>{v.titel}</span>
              <span style={{ color: "#55524c" }}>{" · "}bleibt unverändert</span>
              <div style={{ fontSize: 11.5, color: "#77746c" }}>{v.grund}</div>
            </li>
          ))}
        </ul>
      ...
      {plan.betroffen > 0 && (
        <div style={{ ...hinweis, marginTop: 12 }}>
          {plan.voll.length > 0 && (
            <>
              {plan.voll.length === 1
                ? "Bei 1 Film fällt die Endnote um " + notenText(FILM_SENKUNG_ENDNOTE)
                : "Bei " + plan.voll.length + " Filmen fällt die Endnote um " +
                  notenText(FILM_SENKUNG_ENDNOTE)}
              {" "}— die Rundung der Endnoten-Formel macht daraus vereinzelt bis zu
              {" "}{notenText(FILM_SENKUNG_MAX_REST)} mehr oder weniger.{" "}
            </>
          )}
          Rückgängig machen lässt sich das nicht automatisch.
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
        <button
          type="button"
          onClick={onSenken}
          disabled={busy || !plan.betroffen}
          style={{
            ...knopf, flex: 1,
            background: plan.betroffen ? "var(--accent, #C9A227)" : "transparent",
            color: plan.betroffen ? "#17171A" : "#55524c",
            border: "1px solid " + (plan.betroffen ? "var(--accent, #C9A227)" : "#33333a"),
            cursor: plan.betroffen ? "pointer" : "default",
            opacity: busy ? 0.5 : 1,
          }}
        >
          {plan.betroffen === 1 ? "1 Film absenken" : plan.betroffen + " Filme absenken"}
        </button>
        <button
          type="button"
          onClick={onAbbrechen}
...
export default function App() {
  ...
  /* Die einmalige Absenkung der Filmbewertungen — genau derselbe
     Aufbau: `filmSenkungPlan` ist die durchgerechnete Vorschau,
     solange sie dasteht, `filmSenkungErgebnis` der Satz danach. */
  ...
  /* ---- Alle Filmbewertungen einmalig absenken ----

     Wie bei der Sammelfunktion darueber: zwei getrennte Schritte, und
     der erste fasst nichts an. `filmSenkungVorschau` rechnet nur und
     stellt das Ergebnis hin, `filmSenkungAusfuehren` schreibt — und
     wird ausschliesslich aus dem Bestaetigungsknopf der Vorschau
     heraus gerufen.

     Geschrieben wird Film fuer Film ueber den ganz normalen Weg einer
     Bewertungsaenderung (PUT /api/items), nacheinander und ohne `elo`
     anzufassen. Scheitert einer, laufen die uebrigen trotzdem durch;
     was geglueckt ist, bleibt geglueckt, und die Fehler stehen
     danach da. */
  function filmSenkungVorschau() {
    setFilmSenkungErgebnis("");
    setFilmSenkungPlan(filmSenkungsPlan(items));
  }

  function filmSenkungAbbrechen() {
    setFilmSenkungPlan(null);
  }

  async function filmSenkungAusfuehren() {
    const liste = filmSenkungPlan ? filmSenkungPlan.aenderbar : [];
    if (!liste.length) { setFilmSenkungPlan(null); return; }
    ...
    try {
      for (const vorgang of liste) {
        ...
        /* Noch einmal auf dem Stand gerechnet, der jetzt wirklich
           dasteht: Zwischen Vorschau und Bestaetigung kann sich ein
           Film geaendert haben. */
        const frisch = filmSenkung(current);
        if (!frisch.moeglich) { fehler.push(vorgang.titel + ": " + frisch.grund); continue; }
        try {
          const saved = await api.update(vorgang.id, filmSenkungsAnfrage(current, frisch));
          gespeichert.push(normalizeEntry(saved));
          if (frisch.gekappt) gekappt += 1;
        } catch (e) {
          fehler.push(vorgang.titel + ": " + e.message);
        }
      }
      ...
      if (gespeichert.length) {
        setItems((prev) => {
          const filme = prev[FILM_SENKUNG_KATEGORIE] || [];
          return {
            ...prev,
            [FILM_SENKUNG_KATEGORIE]: filme.map(
              (f) => gespeichert.find((g) => g.id === f.id) || f
            ),
          };
        });
      }

      const zahl = gespeichert.length === 1 ? "1 Film" : gespeichert.length + " Filme";
      setFilmSenkungErgebnis(
        zahl + " abgesenkt" + (gekappt ? ", davon " + gekappt + " gekappt" : "") + "."
      );
      setSaveError(fehler.length ? "Nicht abgesenkt — " + fehler.join("; ") : "");
    } finally {
      setFilmSenkungPlan(null);
      setBusy(false);
    }
    ...
    <div style={{ "--accent": accent, minHeight: "100vh", background: "#17171A", color: "#EDEAE3", fontFamily: "'Inter', system-ui, sans-serif", padding: "0 0 60px 0", overflowX: "clip" }}>
      ...
      <Seite offen={showExport}>
        <div style={{ maxWidth: 720, margin: "0 auto", padding: "20px 20px 0" }}>
          ...
          {/* Die einmalige Absenkung der Filmbewertungen — wie die
              Sammelfunktion darueber ein eigener Kasten, weil sie
              Bewertungen aendert. Sie gilt ausschliesslich fuer
              Filme; deshalb steht die Kategorie im Titel. */}
          <div style={{ background: "#141416", border: "1px solid #2A2A2E", borderRadius: 8, padding: 16, marginBottom: 18 }}>
            <div style={{ fontSize: 12, letterSpacing: 1, color: "var(--accent, #C9A227)", fontFamily: "'JetBrains Mono', monospace", marginBottom: 12 }}>
              FILMBEWERTUNGEN
            </div>
            <FilmSenkung
              plan={filmSenkungPlan}
              busy={busy}
              ergebnis={filmSenkungErgebnis}
              onVorschau={filmSenkungVorschau}
              onSenken={filmSenkungAusfuehren}
              onAbbrechen={filmSenkungAbbrechen}
            />
          </div>
        </div>
      </Seite>
